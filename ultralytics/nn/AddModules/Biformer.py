...
class BiLevelRoutingAttention_nchw(nn.Module):
    ...
    * different pooling behavior (count_include_pad=False)
    ...
    def __init__(self, dim, num_heads=1, n_win=7, qk_scale=None, topk=4, side_dwconv=3, auto_pad=False,
                 attn_backend='torch'):
        super().__init__()
        # local attention setting
        self.dim = dim
...
class HGBlock_Biformer(nn.Module):
    """
    ...
    def __init__(self, c1, cm, c2, k=3, n=6, lightconv=False, shortcut=False, act=nn.ReLU()):
        ...
        self.cv = BiLevelRoutingAttention_nchw(c2)
